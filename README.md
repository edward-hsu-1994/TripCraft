...
6. **安全與目的地研究**：
   ...
   - **旅遊保險**：必備海外突發疾病醫療險（含緊急運送）與旅遊不便險（班機延誤/行李損失）
   - **天候與天災風險**：季節性颱風頻率、大眾運輸停駛應變機制
   - **野生動物與自然危害**：山區景點「熊出沒（熊鈴避難守則）」與火山活動警戒
7. **交通方式** — 步行 / 大眾運輸 / 計程車 / 租車 / 混搭，必問
8. **景點停留時間** — 上網查實際資料，每個景點精確經緯度（`coords: [lat, lng]`）
9. **交通時間 + 等候時間** — 上網查該地大眾運輸實際資料（含路線名稱、班距、步行分鐘、來源 URL）
10. **day-by-day 時間表** — 每一天都是 `Time | Spot | Duration | Transit` 的卡片與路線串接
11. **全天行程 = 停留時間 + 交通時間 + 等候時間 + 用餐 + 緩衝**，不憑感覺拼湊
12. **產出互動式 HTML 網頁** — 單一獨立檔案，自帶 OpenStreetMap 雙向互動、純文字膠囊 Tab 列與總覽三大區塊
