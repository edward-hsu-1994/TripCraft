...
8. **交通方式（多模態移動）** — 步行 / 大眾運輸（鐵道/地鐵/巴士/渡輪） / **公共自行車與觀光單車租借（含電輔車與自行車道）** / 計程車 / 租車 / 混搭，必問
9. **景點停留時間（強制上網查詢）** — 若有聯網工具，必須實查官方或旅客公認之停留時間預估（`建議停留時間 / 所要時間 / visit duration`），並取得精確經緯度（`coords: [lat, lng]`）
10. **交通時間 + 自行車騎行路徑 + 等候時間** — 上網查該地大眾運輸與自行車騎行實際數據（含鐵道路線、自行車騎行距離與時間、班距、步行分鐘、來源 URL）
11. **day-by-day 時間表** — 每一天都是 `Time | Spot | Duration | Transit` 的卡片與路線串接
12. **全天行程 = 停留時間 + 交通時間 + 等候時間 + 用餐 + 緩衝**，不憑感覺拼湊
