...
5. **真實精確數據與強制上網查詢停留時間**：每個景點須包含真實經緯度座標（`coords: [lat, lng]`）、確切交通工具與路線名稱、精確步行時間（如「步行 6 分」）、**自行車租借店家/站點、騎行路線時間與距離（如「於備前一宮站前租借單車 ➔ 騎行 10 分 / 約 2.0km ➔ 吉備津彥神社」）**。**停留時間與單車租賃規定**：若具備聯網查詢工具（Web Search / Browsing），**必須在網路上實際查詢該地點官方或旅客公認之「預估停留時間 / 所要時間 / 建議參觀時間」以及自行車租借店家名稱、營業時間與借還站點**，絕不憑空臆測！
...
- **強制上網查詢停留時間與租借資訊**：若具備網路搜尋功能，**必須搜尋**該景點的預估參觀時間（例如搜尋 `"<景點名稱>" 建議停留時間 OR 所要時間`、`"<車站/景區>" 自行車租借 OR レンタサイクル`），依真實遊覽數據設定（大型古堡/主題樂園 2~3 小時、神社/展望台 45 分~1.5 小時、老街散策 1.5~2 小時）。
