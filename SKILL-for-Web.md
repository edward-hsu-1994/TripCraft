...
1. **確認出發地與範圍**：一律先確認出發地與出遊範圍（國內 vs 國外）。
2. **偏好驅動推薦**：先詢問旅遊風格、步調、預算、氣候、飲食偏好（如熟食為主/不吃生食）、同行夥伴及避忌事項。
3. **推薦 5–8 個目的地**：未定地點時，先提供 5–8 個符合偏好的目的地供使用者挑選。
4. **天數決定景點密度**：天數決定景點數量（2-3天約3-6個、5天約6-10個、7-8天約8-14個）。
...
8. **真實精確數據**：每個景點須包含真實經緯度座標（`coords: [lat, lng]`）、合理停留時間、確切交通工具與路線名稱、精確步行時間（如「步行 6 分」）。
...
12. **對照表嚴格 1:1 衍生**：總覽頁的車站地名日英對照表必須**100% 精準對應行程（Day 1 ~ Day N）中實際造訪的每一個景點、車站、飯店、機場與名店**，嚴禁憑空捏造無關地點。
13. **純前端單一檔案 HTML 輸出**：最終成果輸出為完整的單一 HTML 代碼區塊，內嵌 Vue 3、Tailwind CSS 與 Leaflet OpenStreetMap，使用者下載即可直接雙擊瀏覽器執行。
...
### 第 11 步：核對時間數學與流暢度
確保每日首尾皆為飯店（或機場），營業時間相符，預留通關與劃位緩衝時間。
